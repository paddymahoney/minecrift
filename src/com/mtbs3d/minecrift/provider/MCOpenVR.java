        ...
        IEventNotifier, IEventListener, IBodyAimController
{
    ...
    // Controls
    private static long k_nPlaceBlockButton = (1L << JOpenVRLibrary.EVRButtonId.EVRButtonId_k_EButton_SteamVR_Touchpad);
        ...
        if( mc.currentScreen != null )
        {
                    ...
                    && controllerMouseY>=0 && controllerMouseY<mc.displayHeight)
            {
                ...
                if (controllerDeviceIndex[RIGHT_CONTROLLER] != -1)
                {
                    ...
                    {
                        if ((lastControllerState[RIGHT_CONTROLLER].ulButtonPressed.longValue() & k_nPlaceBlockButton) == 0)
                    ...
                    } else if ((lastControllerState[RIGHT_CONTROLLER].ulButtonPressed.longValue() & k_nPlaceBlockButton) > 0)
                    {
    ...
    private void processControllerButtons()
    {
        ...
        if (mc.currentScreen != null && !sleeping && (controllerStateReference[LEFT_CONTROLLER].ulButtonPressed.longValue() & k_nTeleportButton) > 0)
        {
            mc.thePlayer.closeScreen();
        }
        keyBindTeleport.pressed = !sleeping && (controllerStateReference[LEFT_CONTROLLER].ulButtonPressed.longValue() & k_nTeleportButton) > 0;

        // in restricted mode, update room origin immediately when you release trigger
        if (mc.vrPlayer.restrictedViveClient && (controllerStateReference[LEFT_CONTROLLER].ulButtonPressed.longValue() & k_nTeleportButton) == 0 &&
                (lastControllerState[1].ulButtonPressed.longValue() & k_nTeleportButton) > 0)
        {
    ...
    private void processQuickTorchButton()
    {
        int c = RIGHT_CONTROLLER;
        if (mc.thePlayer != null)
        {
    ...
    {
        final long k_nTouchpadButton = (1L << JOpenVRLibrary.EVRButtonId.EVRButtonId_k_EButton_SteamVR_Touchpad);

        for (int c=0;c<2;c++)
        {
            if (controllerStateReference[c].rAxis[k_EAxis_TouchPad]!=null &&
                    (controllerStateReference[c].ulButtonTouched.longValue() & k_nTouchpadButton) > 0)
            {
    ...
    private void processTouchpadSampleBuffer()
    {
        ...
        {
            final long k_nTouchpadButton = (1L << JOpenVRLibrary.EVRButtonId.EVRButtonId_k_EButton_SteamVR_Touchpad);
            boolean touchpadPressed = (controllerStateReference[c].ulButtonPressed.longValue() & k_nTouchpadButton) > 0;
