...
public class QuaternionHelper
{
    ...
    public static Quaternion pow(Quaternion q1, float power)
    {
        ...
        Vec3 nHat = new Vec3(input.x, input.y, input.z).normalize();
    ...
    public static Quaternion exp(Quaternion input)
    {
        ...
        Vec3 inputV = new Vec3(input.x, input.y, input.z);
        float outputA = (float)(Math.exp(inputA) * Math.cos(inputV.lengthVector()));
        double xCoord = Math.exp(inputA) * (inputV.normalize().xCoord * (float)Math.sin(inputV.lengthVector()));
        double yCoord = Math.exp(inputA) * (inputV.normalize().yCoord * (float)Math.sin(inputV.lengthVector()));
        double zCoord = Math.exp(inputA) * (inputV.normalize().zCoord * (float)Math.sin(inputV.lengthVector()));

        return new Quaternion((float)xCoord, (float)yCoord, (float)zCoord, outputA);
    }
